...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await guestFetch(`/products/${encodeURIComponent(productId)}`);
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setConfirmPolicyError(null);
      try {
        const res = await guestFetch(`/guest/policies/${encodeURIComponent(guestIssuedPolicyId)}`, {
          guestToken: token,
        });
  ...
  const runDummyKyc = useCallback(async () => {
    ...
    try {
      ...
      if (api && token && policyId) {
        try {
          const startRes = await guestFetch(`/guest/policies/${encodeURIComponent(policyId)}/kyc`, {
            method: 'POST',
            guestToken: token,
            body: JSON.stringify({}),
          });
          if (startRes.ok) {
            await guestFetch(`/guest/policies/${encodeURIComponent(policyId)}/kyc/result`, {
              method: 'POST',
              guestToken: token,
              body: JSON.stringify({
                status: 'success',
  ...
  const startPayment = useCallback(async () => {
    ...
    try {
      if (apiBase && token && policyId) {
        const orderRes = await guestFetch(`/guest/policies/${encodeURIComponent(policyId)}/payments/orders`, {
          method: 'POST',
          guestToken: token,
        ...
        if (orderRes.ok) {
          ...
          await openRazorpayCheckout({
            ...
            verify: async (response) => {
              ...
              const verifyRes = await fetch('/api/guest/payments/verify', {
                method: 'POST',
                headers: {
      ...
      if (isClientDirectCheckoutRazorpayAllowed()) {
        ...
        await openRazorpayCheckout({
          ...
          verify: async (response) => {
            const verifyRes = await fetch('/api/razorpay/checkout-verify', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                razorpay_order_id: response.razorpay_order_id,
