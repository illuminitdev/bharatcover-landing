...
import { getGuestApiBase, guestFetch, GUEST_SESSION_STORAGE, USER_SESSION_STORAGE } from '@/lib/guest-api';
...
export default function PolicyViewPage() {
  ...
  const load = useCallback(async () => {
    if (!getGuestApiBase() || (!guestToken && !userToken)) {
    ...
    try {
      const res = guestToken
        ? await guestFetch(`/guest/policies/${encodeURIComponent(policyIdParam)}`, { guestToken })
        : await fetch(`/api/sales/user/policy?policyId=${encodeURIComponent(policyIdParam)}`, {
